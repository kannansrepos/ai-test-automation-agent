...
const TestExecutionModal = ({
  ...
  useEffect(() => {
    if (isOpen && testCases.length > 0) {
      ...
      testCases.forEach((tc) => {
        ...
        initial[tc.id] = {
          testCaseId: tc.id,
          status: tcStatus === 'passed' ? tcStatus : 'idle',

          logs: hasPreviousLogs ? tcLogs : ['Waiting to run...'],

          testScript: (tc as any).browserbaseScript,

          sessionId: (tc as any).sessionId || undefined,

          sessionUrl: (tc as any).sessionUrl || undefined,
        };
      });
      ...
      const hasMissingScript = testCases.some((tc) => !tc.testScript);
  ...
  useEffect(() => {
    ...
    const runTest = async () => {
      ...
      const isRegenerating =
        executionMode === 'generate' || !results[tcId]?.testScript;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-5xl h-[90vh] flex flex-col p-6 gap-4 bg-white rounded-2xl shadow-2xl border overflow-hidden">
        <DialogHeader className="pb-4 flex flex-row items-center justify-between shrink-0">
          <div>
            <DialogTitle className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <PlayCircle className="text-primary h-6 w-6" />
              Browserbase Test Runner
            </DialogTitle>
        ...
        <div className="flex flex-col gap-6 p-4 rounded-2xl border border-gray-200/80 shrink-0">
          <div className="flex flex-col sm:flex-row gap-4 items-end">
            ...
            <div className="flex gap-2.5">
              <Button
                ...
                className={`h-10 px-4 font-medium text-xs gap-1.5 transition-colors border-gray-300 ${
                  showOptions
          ...
          {showOptions && (
            <div className="pt-3 border-t border-gray-200/60 grid grid-cols-1 md:grid-cols-3 gap-5 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <div className="md:col-span-1 space-y-1.5">
                ...
                <div className="grid grid-cols-2 bg-gray-100 p-1 rounded-lg border border-gray-200">
                  <button
                    type="button"
                    disabled={isExecuting}
              ...
              <div className="md:col-span-2 space-y-1.5">
                ...
                <textarea
                  ...
                  disabled={isExecuting}
                  className="w-full rounded-md border border-gray-300 px-3 py-1.5 text-xs font-sans focus:outline-none focus:ring-1 focus:ring-primary disabled:opacity-50 disabled:bg-gray-100 shadow-xs resize-none"
                />
              </div>
            </div>
          )}
            ...
            {currentSelectedTestCase ? (
              <>
                ...
                <div className="p-4 border-b bg-gray-50/50 flex justify-between items-start gap-4 shrink-0">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Expected:
                      {currentSelectedTestCase.expectedResult}
                    </p>
                  </div>
                ...
                <div className="flex-1 flex flex-col p-4 gap-4 overflow-y-auto">
                  ...
                  {currentSelectedResult?.testScript && (
                    <div className="rounded-lg border bg-gray-950 overflow-hidden">
                      ...
                      <pre className="p-3 bg-gray-950 text-neutral-400 font-mono text-[11px] leading-relaxed overflow-x-auto max-h-36">
                        {currentSelectedResult.testScript}
                      </pre>
                    </div>
                  )}
                  ...
                  <div className="flex-1 flex flex-col rounded-lg border overflow-hidden min-h-[48%]">
                    <div className="bg-gray-950 text-gray-200 px-3.5 py-2.5 border-b border-gray-800 flex items-center justify-between shrink-0 font-mono">
                      ...
                      >
                        {currentSelectedResult?.logs?.length || 0} logs
                      </Badge>
                    </div>
